...
#prefix = '/home/lc876/dedalus_mri_code/MRI_Project/data/vasil/POLISHED_DATA/polished_'
#p = Path('.')

prefix = 'polished_'
os.makedirs('/home/lc876/dedalus_mri_code/MRI_Project/png/outward_increase', exist_ok=True)
p = Path('/home/lc876/dedalus_mri_code/MRI_Project/data/outward_increase/POLISHED_DATA/')

dir_list = [str(x) for x in p.iterdir() if x.is_dir() if (x.name.startswith(prefix) & x.name.endswith("Pnone"))]
print(dir_list)
cases = []
for dd in dir_list:
    name = Path(dd).name     # only the directory name
    parts = name.split('_')
...
for kk in range(myrank,ncase,size):
    case = cases[kk]
    print(case)
    eig = np.load(str(p) + '/' + prefix + case + '/eigs.npy')
    γ, ω = eig[0].imag, eig[0].real

    file = h5py.File(str(p) + '/' + prefix + case + '/checkpoints/checkpoints_s1.h5')
    ...
    fig = plt.figure(figsize=[7.3,3.5],dpi=600,constrained_layout=False,linewidth=2.0)
    data = [δΩ, H]
    x, y = np.meshgrid(θ,r)
    ...
    fig.savefig('/home/lc876/dedalus_mri_code/MRI_Project/png/outward_increase/' + case + '.png', bbox_inches='tight')
