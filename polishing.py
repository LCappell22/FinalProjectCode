...
from EVP_Matrices import MRI_Eigenproblem
shear = 1

while shear <= 15:
    os.makedirs('/home/lc876/dedalus_mri_code/MRI_Project/data/cont_shear_2/Shear_mult_{:.1f}/POLISHED_DATA'.format(shear), exist_ok=True)
    os.chdir('/home/lc876/dedalus_mri_code/MRI_Project/data/cont_shear_2/Shear_mult_{:.1f}'.format(shear))
    ...
    Mstr="{:.2f}".format(M)
    kkstr="{:d}".format(kk)
    prefix = "MRI_Model_Fit_M" + Mstr + "_ellmax"+kkstr+"_"
    p = Path('.')
    ...
    kk = ells[1]
    kkstr1="{:d}".format(kk)
    prefix = "MRI_Model_Fit_M" + Mstr + "_ellmax"+kkstr1+"_"
    p1 = Path('.')
    ...
    print(np.min(diffs))
    ploteigs1[np.where((diffs>1e-0) | (ploteigs1.imag<0.001))] = np.inf
    #ploteigs1[np.where((diffs>1e-3))] = np.inf # | (ploteigs1.imag<0.001))] = np.inf
    #print(ploteigs1)
    ndir = len(dir_list1)
    myrank = CW.rank
    size = CW.size
        ...
        for jj in ieigs:
            ...
            if ((np.abs(uparity-1e0)<1e-2) and (np.abs(np.abs(bparity)-1e0)<1e-2)):
                ...
                if first:
                    ...
                    myell = Lmax
                    neigs = 32
                    solver = MRI_Eigenproblem(shear, target_m,myell,mynn,myRe,myM,neigs,guess,MPI.COMM_SELF,label=label)
                    #A = sp.load_npz('A.npz')
                    #B = sp.load_npz('B.npz')
                ...
                data_dir = '/home/lc876/dedalus_mri_code/MRI_Project/data/cont_shear_2/Shear_mult_{:.1f}/POLISHED_DATA/polished_'.format(shear) + label
