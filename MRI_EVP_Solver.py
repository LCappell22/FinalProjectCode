"""
Vasil's MRI Solver code adapted for other systems
"""
...
def MRI_Solver(shear, m, nth, nr, Re, M, neig, guess, comm, label=None):
    ...
    #Output Folder
    os.makedirs('/home/lc876/dedalus_mri_code/MRI_Project/data/cont_shear_2/Shear_mult_{:.1f}'.format(shear), exist_ok=True)
    data_dir = '/home/lc876/dedalus_mri_code/MRI_Project/data/cont_shear_2/Shear_mult_{:.1f}/MRI_Model_Fit_M{:.2f}_ellmax{:d}_m{:d}'.format(shear, np.log10(Mag), Lmax, m)+'_'+label
    ...
    #Distributor
    dist = d3.Distributor(coords, mesh=mesh, dtype=np.complex128, comm=comm)

    #System Basis
    basis = d3.ShellBasis(coords, (Mmax,Lmax,Nmax), radii=radii, dtype=dtype, dealias=(L_dealias, L_dealias, N_dealias))
    basis_k3 = d3.ShellBasis(coords, (Mmax,Lmax,Nmax), radii=radii, dtype=dtype, dealias=(L_dealias, L_dealias, N_dealias), k=3)

    #Basis Components
    b_inner = basis.S2_basis(radius=ri) #Inner sphere boundary conditions
    b_outer = basis.S2_basis(radius=ro) #Outer sphere boundary conditions

    b_merid = basis.meridional_basis #For 2D NCCs
    b_merid_k3 = basis_k3.meridional_basis

    lift_basis = basis.clone_with(k=1) #1st derivative basis
    ...
    eye = d3.Field(dist=dist, bases=(b_merid,b_merid), tensorsig=(coords,coords), dtype=np.complex128, name='eye') #Identity tensor
    ...
    v0['g'][0] = r*np.sin(theta)*(shear * (fz*gt-1)) #Phi component only, shifted to rotating frame
    v0['g'][1] = 0*r
    v0['g'][2] = 0*r
    ...
    solver.solve_sparse(subproblem, neig, guess)#, eigv=eigv, invert=False) #,P=True) whicheig='LM',
    logger.info('solved')
